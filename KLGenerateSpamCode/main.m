...
int main(int argc, const char * argv[]) {
    @autoreleasepool {
        ...
        BOOL needChageConfigFile = NO;//更改配置文件
        ...
        NSString *configFilePath = nil;//配置文件路径
        ...
        for (NSInteger i = 1; i < arguments.count; i++) {
            ...
            if ([argument isEqualToString:@"-changeConfigFile"]) {
                needChageConfigFile = YES;
                configFilePath = arguments[++i];//获取配置文件路径
                continue;
            }
        ...
        if (needChageConfigFile) {
            printf("开始修改配置文件...\n");
            NSMutableArray *arr = [NSMutableArray arrayWithContentsOfFile:configFilePath];
            for (int i = 0 ; i < arr.count; i ++) {
                NSMutableDictionary *dic = [NSMutableDictionary dictionaryWithDictionary:arr[i]];
                NSLog(@"%@",dic);
                [dic setObject:[NSString stringWithFormat:@"%@%@",newClassNamePrefix,dic[@"className"]] forKey:@"className"];
                NSLog(@"%@",dic);
                [arr replaceObjectAtIndex:i withObject:dic];
            }
            BOOL result = [arr writeToFile:configFilePath atomically:YES];
            if (result) {
                NSLog(@"配置文件类名修改成功");
            }else{
                NSLog(@"配置文件类名修改失败");
